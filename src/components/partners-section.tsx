...
const PartnersSection = () => {
  const partners = [{
    ...
    name: "EBRD Star Venture",
    logo: ebrdStarVenture
  }, {
    name: "ITIDA",
    logo: itidaLogo
  }, {
    name: "NilePreneurs",
    logo: nilepreneursLogo
  }, {
    name: "Plug and Play",
    logo: plugandplayLogo
  }, {
    name: "Seedstars",
    logo: seedstarsLogo
  }];
...
Backed by leading global technology and investment partners</h2>
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex">
            ...
            {[...Array(3)].map((_, setIndex) => partners.map((partner, index) => <div key={`${setIndex}-${index}`} className="flex-none w-48 mx-4">
                  <div className="bg-white rounded-xl p-6 shadow-sm hover:shadow-card transition-all duration-300 h-32 flex items-center justify-center">
                    <img src={partner.logo} alt={partner.name} className="max-w-full max-h-full object-contain filter grayscale hover:grayscale-0 transition-all duration-300" />
                  </div>
                </div>))}
          </div>
        </div>
        ...
        <div className="text-center mt-12">
          <div className="inline-block bg-gradient-hero text-white px-6 py-3 rounded-full">
            <span className="font-medium">Award-Winning As a Global Digital Economy Solution 2025 by UNIDO
